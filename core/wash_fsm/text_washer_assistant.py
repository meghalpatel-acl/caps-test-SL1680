...
class TextBasedWashAssistantFSM:
    states = [
        "idle",
        "wait_second_input",
        "wait_confirm_second",
    ...
    def confirm_second_type(self):
        ...
        print(f"{t_jp}を{s_jp}で洗います。よろしいですか？")
    ...
    def ask_maintenance_type(self):
        ...
        print("メンテナンスですね。どの作業をしますか？（ドラムの乾燥/洗浄/カビ取り）")
    ...
    def process_query(self, query):
        ...
        if not query:
            ...
            print("Empty input received, asking user to repeat.")
            # return None
        ...
        if self.state == "idle":
            ...
            if not out1:
                ...
                print("Initial recognition failed, asking user to repeat.")
                return None
            self.intent = out1.get("predicted_intent")
            self.type = out1.get("predicted_type")
            self.intent_conf = out1.get("intent_confidence")
            self.type_conf = out1.get("type_confidence")
            ...
            if self.intent == "maintenance":
                print(f"Current intent: {self.intent}")
                if self.type_conf and self.type_conf > SIMILARITY_THRESHOLD_TYPE and self.type in maintenance_json:
                    ...
                    # self._speak("メンテナンスですね。どの作業をしますか？（ドラムの乾燥/洗浄/カビ取り）")
                    print("Maintenance intent detected but type confidence below threshold or type not recognized, asking for maintenance type.")
            ...
            # Handle general_info intent (embedding QA only, no model)
            elif self.intent == "general_info" or not self.intent_conf or self.intent_conf < SIMILARITY_THRESHOLD:
                ...
                #     print(f"Agent (Claude): {claude_resp}")
                # else:
                #     # self._speak(answer)
                #     print(f"Answer: {answer} (Similarity: {similarity:.3f})")
                # self.reset()
                return None

            # Only proceed if both intent and type confidence are above threshold for wash intent
            elif self.intent_conf < SIMILARITY_THRESHOLD or self.type_conf < SIMILARITY_THRESHOLD_TYPE:
                # self._speak("もう一度お願いします。")
                print("Intent or type confidence below threshold, asking user to repeat.")
                # return None
                return response

            # if second type is "none" for wash intent
            elif self.type == "none" or not self.type:
                # self._speak("オプションの特定ができませんでした。もう一度詳細をお願いします。")
                print("Type is 'none' or not detected, asking for details again.")
                # return None
                return response
            ...
            if not out2:
                ...
                print("Second stage recognition failed, asking user to repeat.")
                return None
            second_type = out2.get("predicted_second_type")
            self.second_type_conf = out2.get("second_type_confidence")
            response = out2
            ...
            if not out2:
                ...
                print("Confirmation recognition failed, asking user to repeat.")
                return None
            result = out2.get("predicted_second_type")
            response = out2
            if result in {"yes"}:
                command_label = self.resolve_command_label()
                if not command_label:
                    # self._speak("洗濯コースを特定できませんでした。最初からやり直してください。")
                    print("Could not resolve command label, resetting FSM.")
                    self.reset()
                    # return None
                    return response
                self.confirm_yes()
            elif result in "no":
                # self._speak("わかりました。もう一度、どのように洗うか教えてください。")
                self.confirm_no()
            elif result in  "back":
                self.go_back()
            else:
                # self._speak("はい / いいえ で答えてください。")
                print("Unrecognized confirmation response, asking user to answer yes or no.")
            # return None
            return response
            ...
            if not out1:
                ...
                print("Maintenance type recognition failed, asking user to repeat.")
                # return None
            self.type = out1.get("predicted_type")
            self.type_conf = out1.get("type_confidence")
            response = out1
            if self.type_conf and self.type_conf > SIMILARITY_THRESHOLD_TYPE and self.type in maintenance_json:
                ...
                print("Maintenance type confidence below threshold or type not recognized, asking user to specify maintenance type again.")
            # return None
            return response
