...
class MoonshineSynap(BaseSpeechToTextModel):
    def __init__(
        ...
        *,
        ...
        local_model_dir: str = "jp-models/moonshine_jp"
    ):
        super().__init__(
            hf_repo,
        ...
        # Always use local config/tokenizer from local_model_dir
        self.config_path = f"{local_model_dir}/config.json"
        self.tokenizer_path = f"{local_model_dir}/tokenizer.json"
        print(f"Using config: {self.config_path}")
        print(f"Using tokenizer: {self.tokenizer_path}")
        import json
        from tokenizers import Tokenizer
        self.config = json.load(open(self.config_path, "r", encoding="utf-8"))
        self.tokenizer = Tokenizer.from_file(self.tokenizer_path)

        encoder_onnx_path = f"{local_model_dir}/encoder_model.onnx"
        self.encoder_onnx = onnxruntime.InferenceSession(
            encoder_onnx_path,
            providers=['CPUExecutionProvider']
        )
        # Try to load local synap files if they exist, otherwise fallback to download
        import os
        encoder_synap_path = f"{local_model_dir}/encoder.synap"
        decoder_uncached_synap_path = f"{local_model_dir}/decoder_uncached.synap"
        decoder_cached_synap_path = f"{local_model_dir}/decoder_cached.synap"
        if os.path.exists(encoder_synap_path):
            self.encoder = Network(encoder_synap_path)
        else:
        ...
        if os.path.exists(decoder_uncached_synap_path):
            self.decoder_uncached = Network(decoder_uncached_synap_path)
        else:
        ...
        if os.path.exists(decoder_cached_synap_path):
            self.decoder_cached = Network(decoder_cached_synap_path)
        else:
    ...
    def _generate(self, audio: np.ndarray, max_len: int | None = None) -> np.ndarray:
        ...
        dec_st = time.time()
        i = 0  # Fix: ensure i is always defined
...
class MoonshineOnnx(BaseSpeechToTextModel):
    ...
    def __init__(
        ...
        *,
        ...
        # Always use local_model_dir for ONNX models
        encoder_path = f"{local_model_dir}/encoder_model.onnx"
        decoder_path = f"{local_model_dir}/decoder_model_merged.onnx"
...
def main():
    ...
    parser.add_argument(
        "--local-model-dir",
        type=str,
        help="Path to local model directory (overrides HuggingFace download)"
    )
    ...
    if model_type == "onnx":
        ...
        stt = MoonshineOnnx(
            model_size=model_size, quant_type=model_quant, n_threads=args.threads, local_model_dir=args.local_model_dir
        )
    elif model_type == "synap":
        print("Loading Moonshine model using SyNAP runtime ...")
        stt = MoonshineSynap(
            model_size=model_size, quant_type=model_quant, local_model_dir=args.local_model_dir
        )
    else:
