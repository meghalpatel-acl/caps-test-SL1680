...
class StatsDelta:
    ...
    @staticmethod
    def _idle_ticks(stats: list[int]) -> int:
        idle = stats[3] if len(stats) > 3 else 0
        iowait = stats[4] if len(stats) > 4 else 0
        return idle + iowait
    
    @classmethod
    def from_snaps(
        cls, new_snap: StatsSnapshot, old_snap: StatsSnapshot
    ) -> "StatsDelta":
        d_cpu_total: dict[str, int] = {}
        d_cpu_idle: dict[str, int] = {}
        for proc, new in new_snap.cpu.items():
            ...
            d_cpu_idle[proc] = cls._idle_ticks(new) - cls._idle_ticks(old)
        return cls(
...
class StatsCollector:
    ...
    def __init__(
        ...
        if self._mem_total_kb is None:
            raise ValueError(
                f"Failed to parse total available memory from '{self._mem_info}'"
            )
    ...
    def collect_once(self) -> StatsSnapshot:
        ...
        with self._mem_info.open() as f:
            for line in f:
                ...
                logger.warning(
                    f"Failed to parse available available memory from '{self._mem_info}'"
                )
        try:
            npu_time_us = int(self._npu_info.read_text())
        except (FileNotFoundError, PermissionError):
            npu_time_us = 0  # Default to 0 if hardware stats are unavailable
        return StatsSnapshot(time.monotonic(), cpu_stats, npu_time_us, mem_avail_kb)


class SystemProfiler:

    def __init__(self, *, interval_ms: int = 100, hist_length: int = 100):
    ...
    def curr_cpu_usage(self) -> dict[str, float]:
        with self._lock:
            return {
                cpu_name: hist[-1][1] if hist else 0.0
    ...
    def curr_npu_usage(self) -> float:
        with self._lock:
            return self._npu_hist[-1][1] if self._npu_hist else 0.0
    ...
    def avg_cpu_usage(self) -> dict[str, float]:
        with self._lock:
            return {
                cpu_name: (sum(val[1] for val in hist) / len(hist)) if hist else 0.0
                for cpu_name, hist in self._cpu_hist.items()
            }
    ...
    def avg_npu_usage(self) -> float:
        with self._lock:
            return (
                (sum(val[1] for val in self._npu_hist) / len(self._npu_hist)) if self._npu_hist else 0.0
            )

    def get_avg_cpu_during_window(self, start_time: float, end_time: float) -> dict[str, float]:
        """Get average CPU usage during time window [start_time, end_time)"""
        with self._lock:
            result = {}
            for cpu_name, hist in self._cpu_hist.items():
                samples_in_window = [val[1] for val in hist if start_time <= val[0] < end_time]
                result[cpu_name] = (sum(samples_in_window) / len(samples_in_window)) if samples_in_window else 0.0
            return result

    def get_avg_npu_during_window(self, start_time: float, end_time: float) -> float:
        """Get average NPU usage during time window [start_time, end_time)"""
        with self._lock:
            samples_in_window = [val[1] for val in self._npu_hist if start_time <= val[0] < end_time]
            return (sum(samples_in_window) / len(samples_in_window)) if samples_in_window else 0.0
    ...
    def start(self):
        """Starts the background monitoring threads."""
    ...
    def stop(self, *, timeout: float | None = None):
        """Stops the background monitoring threads."""
        self._stop_evt.set()   
            ...
            if self._prev_stats:
                ...
                if d_time_s:  # avoid div-by-zero for <1 ms dt
                    for proc in diff.d_cpu_total:
                        ...
                        with self._lock:
                            self._cpu_hist[proc].append((curr_stats.ts, cpu_usage))
                    npu_usage = (
                        curr_stats.npu_time_us - self._prev_stats.npu_time_us
                    ) / (d_time_s * 1e6)
                    npu_usage = min(max(0.0, npu_usage), 1.0)
                    mem_usage = self._total_mem - curr_stats.mem_avail_kb
                    with self._lock:
                        self._npu_hist.append((curr_stats.ts, npu_usage))
