...
try:
    import importlib
    jp_moonshine_module = importlib.import_module("core.speech_to_text.jp-moonshine")
    JPMoonshineOnnx = getattr(jp_moonshine_module, "JPMoonshineOnnx")
except (ImportError, AttributeError) as e:
    print(f"Error importing JP Moonshine model: {e}")
    sys.exit(1)

# Single model name for JP Moonshine
MODEL_NAME: Final = "jp-moonshine-onnx"

class MoonshineJPProfiler(ProfilerBase):
    def __init__(self, model_names, n_threads=1):
        logger = logging.getLogger("MoonshineJPProfiler")
    ...
    def _load_models(self, model_names):
        """Load JP Moonshine ONNX model"""
        # JP Moonshine only has one model variant
        ...
        try:
            ...
            for model_name in model_names:
                try:
                    self.models[model_name] = JPMoonshineOnnx(rate=16000, n_threads=self._n_threads)
                    self._logger.info(f"Loaded JP Moonshine model: {model_name}")
...
def load_wav_files(wav_dir, num_files=15):
...
if __name__ == "__main__":
    ...
    wav_files_path = os.path.join(PROJECT_ROOT,args.audio_dir)
    print(f"\n>>> Loading/generating audio files from: {wav_files_path} ...")
    wav_files = load_wav_files(wav_files_path, args.num_audio_files)
